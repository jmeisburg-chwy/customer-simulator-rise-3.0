...
All 30 tests passed most recently after the scripted chat response, frontend progression fix, chat scenario id correction to `late_delivery_20_partial_refund_chat`, voice customer beat fix, on-time delivery chat sequence update, current-step guard against adding later scripted beats, off-path chat recovery guard, and server-side step-pass inference for older chat clients.
