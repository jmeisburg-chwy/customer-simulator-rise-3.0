...
- The `/chat-turn` handler defaults missing `stepPassed` to `true` for old Rise blocks, but new Rise HTML sends the explicit value. `text.verbosity` is now `"high"` only when a scripted response exists and `stepPassed` is true; off-path recovery turns stay `"low"`.
...
- The current tests cover scenario normalization, preservation of `customerResponse` through legacy step normalization, mirroring `chatConfig.stepProgression` into the state model, `buildScenarioClientConfig()` preserving scripted responses, the scripted-response guard against premature closes and later-beat leakage, the chat frontend sending the matched step and `stepPassed` to Lambda before advancing progression, and the off-path prompt guard that withholds scripted responses when `stepPassed` is false. They do not directly exercise the live `/chat-turn` `textVerbosity` branch.
...
All 29 tests passed most recently after the scripted chat response, frontend progression fix, chat scenario id correction to `late_delivery_20_partial_refund_chat`, voice customer beat fix, on-time delivery chat sequence update, current-step guard against adding later scripted beats, and off-path chat recovery guard.
