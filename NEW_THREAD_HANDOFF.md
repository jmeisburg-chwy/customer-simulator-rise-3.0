...
- `buildChatInstructions(s, currentStep, options)` looks up the current step from `simulation.stateModel.chatStepProgression` first, then `chatConfig.stepProgression`, by exact `id` or array index.
- When the current step has `customerResponse` and the learner passed the current step, `buildChatInstructions()` injects a `SCRIPTED RESPONSE RULE` block telling the model to use that response exactly or extremely closely, not shorten it, not summarize it, and not remove details such as pet name, breed, birthday, address, timing concern, refund preference, or closing appreciation.
- The chat style rule now allows longer responses when a manager-approved scripted response is active for the current turn.
- The `/chat-turn` handler independently checks the current step for `customerResponse` and sends `text.verbosity: "high"` to OpenAI for active scripted turns; otherwise it keeps `text.verbosity: "low"`.
- The `/chat-turn` handler now deterministically returns the current step's manager-approved `customerResponse` whenever a scripted response is active. It still calls OpenAI for schema/current-step handling, but `resolveChatCustomerMessage()` prevents the final response from leaking later scripted beats if the model generates extra text such as `It's 1234 Elm Street in El Paso. What happens if this order doesn't arrive on time?`.
...
- `ArticulateRise-ChatExperience.html` now sends `stepPassed` in the `/chat-turn` payload. This lets Lambda distinguish a learner message that earned the current scripted customer response from an off-path or nonsensical learner message that should stay on the same beat.
- `buildChatInstructions(s, currentStep, { stepPassed })` now withholds `SCRIPTED RESPONSE RULE` when `stepPassed === false` and injects an `OFF-PATH RESPONSE RULE` instead. The off-path rule tells the customer to react naturally, stay on the current beat, avoid revealing later facts or scripted beats, and prompt the learner to recover.
- The `/chat-turn` handler now infers `stepPassed` from the current step match rules when older Rise blocks omit it. New Rise HTML still sends the explicit value. `text.verbosity` is now `"high"` only when a scripted response exists and `stepPassed` is true; off-path recovery turns stay `"low"`.
- Live debug note: posting `latestAgentMessage: "this is amazon wrong company!"` to scenario 1 with `stepPassed:false` returned an off-path correction, while omitting `stepPassed` returned the scripted first beat on the previously deployed Lambda. This proved stale/old-client behavior was the cause of the screenshot where the customer said "Thank you. I just want to be sure it gets here on time." to an obviously wrong-company learner message.
...
- The current tests cover scenario normalization, preservation of `customerResponse` through legacy step normalization, mirroring `chatConfig.stepProgression` into the state model, `buildScenarioClientConfig()` preserving scripted responses, the scripted-response guard against premature closes and later-beat leakage, the deterministic `resolveChatCustomerMessage()` override for active scripted responses, the chat frontend sending the matched step and `stepPassed` to Lambda before advancing progression, the off-path prompt guard that withholds scripted responses when `stepPassed` is false, and Lambda's server-side step-pass inference for older clients that omit `stepPassed`. They do not directly exercise the live `/chat-turn` `textVerbosity` branch.
...
All 31 tests passed most recently after the scripted chat response, frontend progression fix, chat scenario id correction to `late_delivery_20_partial_refund_chat`, voice customer beat fix, on-time delivery chat sequence update, current-step guard against adding later scripted beats, deterministic scripted-response override, off-path chat recovery guard, and server-side step-pass inference for older chat clients.
